...
pub struct KvEventIndex {
    ...
    /// duplicate batches and detects regressions or gaps. A regression or gap
    /// clears the worker's cached ownership before applying the new batch so
    /// missing removals cannot leave stale-positive owners. Cleared on
    /// `remove_worker` because a re-added worker may legitimately have a fresh
    /// publisher whose sequence numbers restart from zero.
...
/// Drain `WorkerEvent`s and apply each batch to the tree. Duplicate
/// (seq == last_applied) and stale (worker not in `live_workers`) batches are
/// skipped. A sequence regression is treated as an implicit publisher restart.
/// A forward gap is treated as lossy PUB delivery; both cases fail closed by
/// clearing the worker's existing ownership before applying the visible batch.
/// Current SGLang publishers send END_SEQ only on the replay DEALER socket, not
/// the PUB stream consumed here.
async fn pump_loop(
    tree: Arc<HashTree>,
    cursors: Arc<Mutex<HashMap<KvWorkerId, i64>>>,
    live_workers: Arc<Mutex<HashSet<KvWorkerId>>>,
                ...
                if let Some(p) = prev {
                    if seq == p {
                        debug!(
                            ...
                            seq,
                            last_applied = p,
                            "kv-events pump: duplicate batch; skipping",
                        );
                        continue;
                    }
                    if seq < p {
                        // A single SUB task preserves per-publisher message
                        // order. Therefore a strict sequence regression means
                        // the publisher process restarted. Clear stale cache
                        // ownership before accepting the first visible batch
                        // of the new generation; it need not be seq=1 because
                        // PUB/SUB slow-join can drop early messages.
                        info!(
                            worker = ?worker,
                            seq,
                            last_applied = p,
                            "kv-events pump: publisher sequence regressed; resetting worker state",
                        );
                        tree.clear_worker(&worker);
                    } else if seq > p.saturating_add(1) {
                        warn!(
                            worker = ?worker,
                            seq,
                            last_applied = p,
                            first_missing = p.saturating_add(1),
                            "kv-events pump: publisher sequence gap; clearing stale worker state before applying visible batch",
                        );
                        tree.clear_worker(&worker);
                    }
                }
                for event in &batch.events {
                    match event {
                        ...
                        KvCacheEvent::BlockRemoved(b) => {
                            tree.remove_with_medium(&worker, &b.block_hashes, b.medium.as_deref());
                        }
...
mod tests {
    ...
    /// A sequence regression means the publisher process restarted without
    /// broadcasting END_SEQ on its PUB socket. The stale ownership tree must
    /// be cleared before accepting the new publisher generation.
    #[tokio::test]
    async fn pump_resets_worker_when_sequence_regresses() {
        let id = worker_id("http://w1", 0);
        let h = spawn_pump(std::slice::from_ref(&id));
        ...
        .await
        .unwrap();
        // A freshly restarted publisher begins below the previous cursor.
        // Its first visible batch may be greater than 1 because a SUB socket
        // can miss early messages while reconnecting.
        tx.send(WorkerEvent::Batch {
            worker: id.clone(),
            seq: 3,
            batch: batch(vec![KvCacheEvent::BlockStored(BlockStored {
        ...
        assert_eq!(
            tree.match_prefix(None, &[10]).matched_blocks,
            0,
            "old publisher generation must be cleared",
        );
        assert_eq!(tree.match_prefix(None, &[20]).matched_blocks, 1);
        assert_eq!(cursors.lock().get(&id).copied(), Some(3));
    }

    /// An equal sequence number is a duplicate from the same generation and
    /// must remain a no-op; it is not evidence of a publisher restart.
    #[tokio::test]
    async fn pump_filters_duplicate_sequence() {
        let id = worker_id("http://w1", 0);
        let h = spawn_pump(std::slice::from_ref(&id));
        let (tree, cursors, tx, pump) = (h.tree, h.cursors, h.tx, h.pump);
    ...
    #[tokio::test]
    async fn pump_keeps_cpu_owner_after_gpu_removal() {
        let id = worker_id("http://w1", 0);
        let h = spawn_pump(std::slice::from_ref(&id));
        let (tree, cursors, tx, pump) = (h.tree, h.cursors, h.tx, h.pump);
        ...
        let matched = tree.match_prefix(None, &[10, 20]);
        assert_eq!(matched.matched_blocks, 2);
        assert_eq!(matched.workers, HashSet::from([id.clone()]));
        assert_eq!(cursors.lock().get(&id).copied(), Some(1));
    }

    #[tokio::test]
    async fn pump_clears_stale_worker_state_on_sequence_gap() {
        let id = worker_id("http://w1", 0);
        let h = spawn_pump(std::slice::from_ref(&id));
        let (tree, cursors, tx, pump) = (h.tree, h.cursors, h.tx, h.pump);
        ...
        assert_eq!(
            tree.match_prefix(None, &[10]).matched_blocks,
            0,
            "state predating an unobserved batch must be cleared",
        );
        assert_eq!(tree.match_prefix(None, &[20]).matched_blocks, 1);
        assert_eq!(cursors.lock().get(&id).copied(), Some(3));
    }
