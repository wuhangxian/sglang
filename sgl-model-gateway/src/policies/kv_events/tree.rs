...
//! tracks the storage media held by each [`KvWorkerId`] for the chain ending
//! at that node. A worker is an owner while at least one medium remains.
//!
...
/// Storage-medium identity carried by SGLang KV events.
///
/// Residency is a set, not an exclusive state: a HiCache block normally moves
/// through `GPU -> {GPU, CPU_PINNED} -> CPU_PINNED`. `Unspecified` preserves
/// legacy events whose optional `medium` field was omitted without pretending
/// that they describe a particular tier. Unknown future media remain distinct
/// so a removal for one value cannot accidentally erase another.
#[derive(Clone, Eq, Hash, PartialEq, Debug)]
enum KvStorageMedium {
    Unspecified,
    Gpu,
    CpuPinned,
    Disk,
...
impl From<Option<&str>> for KvStorageMedium {
    fn from(medium: Option<&str>) -> Self {
        match medium {
            None => Self::Unspecified,
            Some("GPU") => Self::Gpu,
            Some("CPU_PINNED") => Self::CpuPinned,
            Some("DISK") => Self::Disk,
            Some("EXTERNAL") => Self::External,
            Some(other) => Self::Other(Arc::from(other)),
        }
    }
}
...
struct Node {
    ...
    /// Per-worker set of independently present storage media. The map keys
    /// are the worker-owner union exposed by the public match APIs.
    workers: HashMap<KvWorkerId, HashSet<KvStorageMedium>>,
...
/// * Each node's owner union is the key set of `Node::workers`; each value is
///   the set of independently present media for that worker.
/// * Pruning runs after a medium removal empties the final worker entry on a
///   node: prune detaches from parent, removes from `by_hash`, and recurses
///   upward.
#[derive(Debug)]
struct TreeState {
...
impl TreeState {
    ...
    fn remove(&mut self, worker: &KvWorkerId, medium: &KvStorageMedium, block_hashes: &[i64]) {
        ...
        for id in targets {
            ...
            let still_present = match self.nodes.get_mut(&id) {
                Some(node) => {
                    if let Some(media) = node.workers.get_mut(worker) {
                        media.remove(medium);
                        if media.is_empty() {
    ...
    fn match_prefix(&self, parent_hash: Option<i64>, block_hashes: &[i64]) -> MatchResult {
        ...
        let workers = match last_match_node {
            Some(id) => self
                .nodes
                .get(&id)
                .map(|n| n.workers.keys().cloned().collect())
                .unwrap_or_default(),
            None => HashSet::new(),
        };
        MatchResult {
    ...
    /// Return the deepest matching prefix held by every worker encountered
    /// while walking the requested hash chain.
    ///
    /// Unlike `match_prefix`, which only returns the worker set on the
    /// deepest globally matched node, P2P routing needs to compare cache
    /// ownership across all registered workers. A worker that owns three
    /// blocks must remain visible even when another worker owns five.
    fn match_prefix_by_worker(
        &self,
        parent_hash: Option<i64>,
        block_hashes: &[i64],
        ...
        let mut current = start;
        let mut matched_by_worker: HashMap<KvWorkerId, usize> = HashMap::new();
        let now = now_millis();
        for (depth, &block_hash) in block_hashes.iter().enumerate() {
            let Some(child_id) = self
                .nodes
                .get(&current)
                .and_then(|node| node.children.get(&block_hash).copied())
            else {
                break;
            };
            ...
            child.last_used.store(now, Ordering::Relaxed);
            let matched_blocks = depth + 1;
            for worker in child.workers.keys() {
                matched_by_worker
                    .entry(worker.clone())
                    .and_modify(|best| *best = (*best).max(matched_blocks))
                    .or_insert(matched_blocks);
            }
            current = child_id;
        }
...
impl HashTree {
    ...
    /// Apply a legacy `BlockStored` event whose optional medium was omitted.
    ///
    ...
    pub fn insert(&self, worker: &KvWorkerId, parent_hash: Option<i64>, block_hashes: &[i64]) {
        self.insert_with_medium(worker, parent_hash, block_hashes, None);
    }

    /// Apply a `BlockStored` event for one storage medium.
    ///
    /// Repeated stores for the same `(worker, medium)` are idempotent. A worker
    /// remains in the owner union until every independently stored medium has
    /// been removed.
    pub fn insert_with_medium(
        &self,
        worker: &KvWorkerId,
        parent_hash: Option<i64>,
        block_hashes: &[i64],
        ...
        let medium = KvStorageMedium::from(medium);
        let mut state = self.state.write();
        state.insert(worker, &medium, parent_hash, block_hashes);
    }

    /// Apply a legacy `BlockRemoved` event whose optional medium was omitted.
    ///
    /// For every node carrying any hash in `block_hashes`, drop the worker's
    /// legacy-unspecified residency. Nodes that become empty AND childless are
    /// pruned (cascading upward).
    ///
    /// Removing this residency does NOT remove the node if another medium or
    /// worker still holds it.
    pub fn remove(&self, worker: &KvWorkerId, block_hashes: &[i64]) {
        self.remove_with_medium(worker, block_hashes, None);
    }

    /// Apply a `BlockRemoved` event for one storage medium.
    ///
    /// Only the specified medium is removed. The worker remains an owner when
    /// another medium is still present, and repeated removals are idempotent.
    pub fn remove_with_medium(
        &self,
        worker: &KvWorkerId,
        block_hashes: &[i64],
        medium: Option<&str>,
        ...
        let medium = KvStorageMedium::from(medium);
        let mut state = self.state.write();
        state.remove(worker, &medium, block_hashes);
    }
    ...
    /// Find the deepest matching prefix for each worker on the path.
    ///
    /// This is used by Prefill-to-Prefill routing to identify the real
    /// source worker before deciding whether load imbalance justifies a
    /// transfer to another Prefill.
    pub fn match_prefix_by_worker(
        &self,
        parent_hash: Option<i64>,
        block_hashes: &[i64],
    ) -> HashMap<KvWorkerId, usize> {
        self.state
            .read()
            .match_prefix_by_worker(parent_hash, block_hashes)
    }
...
mod tests {
    ...
    #[test]
    fn removing_gpu_keeps_worker_owned_through_cpu_medium() {
        let tree = HashTree::new();
        let a = worker("http://a", 0);

        tree.insert_with_medium(&a, None, &[1, 2, 3], Some("GPU"));
        tree.insert_with_medium(&a, None, &[1, 2, 3], Some("CPU_PINNED"));
        tree.remove_with_medium(&a, &[1, 2, 3], Some("GPU"));

        let by_worker = tree.match_prefix_by_worker(None, &[1, 2, 3]);
        assert_eq!(by_worker.get(&a), Some(&3));
        assert_eq!(
            tree.match_prefix(None, &[1, 2, 3]).workers,
            workers(&[&a]),
            "CPU_PINNED residency must keep the worker in the owner union",
        );

        tree.remove_with_medium(&a, &[1, 2, 3], Some("CPU_PINNED"));
        assert_eq!(tree.match_prefix(None, &[1, 2, 3]).matched_blocks, 0);
        assert_eq!(tree.node_count(), 0);
    }

    #[test]
    fn repeated_store_for_same_medium_is_idempotent() {
        let tree = HashTree::new();
        let a = worker("http://a", 0);

        tree.insert_with_medium(&a, None, &[1, 2], Some("GPU"));
        tree.insert_with_medium(&a, None, &[1, 2], Some("GPU"));
        tree.remove_with_medium(&a, &[1, 2], Some("GPU"));

        assert_eq!(
            tree.match_prefix(None, &[1, 2]).matched_blocks,
            0,
            "duplicate state assertions must not create an event-count refcount",
        );
        tree.remove_with_medium(&a, &[1, 2], Some("GPU"));
        assert_eq!(tree.node_count(), 0, "duplicate removal must be a no-op");
    }

    #[test]
    fn legacy_unspecified_and_named_mediums_are_independent() {
        let tree = HashTree::new();
        let a = worker("http://a", 0);

        tree.insert(&a, None, &[1]);
        tree.insert_with_medium(&a, None, &[1], Some("GPU"));
        tree.remove_with_medium(&a, &[1], Some("GPU"));
