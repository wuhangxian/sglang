...
pub async fn fetch_event_config(
    ...
    let block = match body.kv_events {
        Some(b) => b,
        None => {
            match legacy_kv_events_block(body.kv_events_config, body.page_size, body.dp_size)? {
                Some(block) => {
                    debug!(
                        worker_url = worker_url,
                        "kv-events discovery: using legacy kv_events_config"
                    );
                    block
                }
                None => {
                    debug!(
                        worker_url = worker_url,
                        "kv-events discovery: /server_info has neither kv_events nor a usable \
                     kv_events_config; worker is not publishing"
                    );
                    return Ok(None);
                }
            }
...
struct ServerInfoResponse {
    ...
    /// xSGL/SGLang 0.5.6-style workers expose the original CLI JSON value
    /// rather than the newer normalized `kv_events` block.
    #[serde(default)]
    kv_events_config: Option<LegacyKvEventsConfigField>,
    #[serde(default)]
...
#[derive(Deserialize)]
#[serde(untagged)]
enum LegacyKvEventsConfigField {
    JsonString(String),
    Object(LegacyKvEventsConfig),
}

#[derive(Clone, Deserialize)]
struct LegacyKvEventsConfig {
    #[serde(default)]
...
fn legacy_kv_events_block(
    field: Option<LegacyKvEventsConfigField>,
    page_size: Option<u32>,
    dp_size: Option<u32>,
) -> Result<Option<KvEventsBlock>> {
    let Some(field) = field else {
        return Ok(None);
    };
    let config = match field {
        LegacyKvEventsConfigField::JsonString(raw) => {
            serde_json::from_str::<LegacyKvEventsConfig>(&raw)
                .map_err(|e| anyhow!("legacy kv_events_config JSON parse failed: {e}"))?
        }
        LegacyKvEventsConfigField::Object(config) => config,
    };

    if config
        .publisher
        .as_deref()
        .is_some_and(|publisher| !publisher.eq_ignore_ascii_case("zmq"))
    {
        return Ok(None);
    }
    ...
    let endpoint = config
        .endpoint
        .strip_prefix("tcp://")
        .ok_or_else(|| anyhow!("legacy kv_events_config endpoint must use tcp://"))?;
    let (host, port) = endpoint
        .rsplit_once(':')
        .ok_or_else(|| anyhow!("legacy kv_events_config endpoint has no port"))?;
    let host = host.trim_start_matches('[').trim_end_matches(']');
    if host.is_empty() {
        return Err(anyhow!("legacy kv_events_config endpoint has no host"));
    }
    let port_base = port
        .parse::<u16>()
        .map_err(|e| anyhow!("legacy kv_events_config endpoint has invalid port: {e}"))?;
    let block_size =
        page_size.ok_or_else(|| anyhow!("legacy kv_events_config requires top-level page_size"))?;
    let dp_size = dp_size.unwrap_or(1);
    if dp_size == 0 {
        return Err(anyhow!("legacy kv_events_config has invalid dp_size=0"));
    }

    Ok(Some(KvEventsBlock {
...
mod tests {
    ...
    /// xSGL 0.5.6-style workers expose the original CLI JSON string plus
    /// top-level page/DP sizes instead of the newer structured block.
    #[tokio::test]
    async fn fetch_returns_event_config_from_legacy_kv_events_config() {
        let body = Arc::new(json!({
            "kv_events_config": "{\"publisher\":\"zmq\",\"endpoint\":\"tcp://*:32000\",\"topic\":\"kv\"}",
            "page_size": 64,
            "dp_size": 2,
        }));
        let (url, _shutdown) = spawn_fake_worker(body).await;
